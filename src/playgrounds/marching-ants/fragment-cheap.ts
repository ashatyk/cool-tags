...
void main(){
  ...
  float phase  = (dot(p, dir) + uStripeSpeedPx*uTime) / period;
  float aa     = fwidth(phase);
  float s      = sin(6.2831853*phase);
  float k      = clamp(uStripeSmooth, 0.0, 1.0);
  float soft   = max(k, aa);                       // было k + aa
  float stripe = smoothstep(-soft, soft, s);       // 0..1                       // 0..1

  vec3 col  = mix(uColorOff, uColorOn, stripe);
    
  fragColor = vec4(col * band*band, band*uIntensity);
}
`;
